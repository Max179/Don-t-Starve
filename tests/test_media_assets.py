...
def test_rebuild_entity_media_assets_can_restore_after_page_image_cascade(tmp_path):
    conn = connect(tmp_path / "wiki.sqlite")
    init_db(conn)
    source_id = upsert_source(
        conn,
    ...
    metadata = capture_entity_media_asset_metadata(conn)
    ...
    rebuild_entity_media_assets(conn, preserved_metadata=metadata)
