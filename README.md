...
Current committed output: `data/dont_starve_wiki.sqlite` contains 2,593 English-first wiki entities, 5,483/5,483 matched wiki.gg + Fandom page-index rows with zero remaining source-page gaps, 4,793 entity-source profile rows, 2,593 entity-source coverage rows, 393 entity source-gap queue rows, 27,041 parsed attributes, 7,534 normalized stat rows, 21,254 unified media asset rows, 1,457 media profile rows, 2,593 media coverage rows, 3,988 media gap queue rows, 501 image-variant candidates, 62,839 wiki-link relation rows, 46,538 resolved wiki-link targets, 5,150 typed gameplay relationship edges, 25,916 taxonomy tags, 14,209 category links, 9,989 identity keys, 3,430 merged variant summaries, 2,593 completeness audit rows, 9,217 completeness gap queue rows, 2,412 official verification queue rows, 2,315 structured recipe ingredients, 1,337 structured drop/source/sold/spawn facts, 163 official Steam/Klei verification records, 56 normalized official product records, 226 official product media URLs, 50 normalized official update events, 80 normalized official update sections, 397 official update section items, 735 official-record entity mentions, 8 ranked source catalog rows, 30 source catalog evidence rows, 12 representative source topic probes, 21 curated community guide source rows, 77 community guide topic rows, 5 community guide topic-index rows, and 11 source-access audit records. The official layer includes appdetails for Don't Starve, Don't Starve Together, Don't Starve Elsewhere, and all 53 Steam-listed DLC ids, with official header, capsule, and description image URLs. See [docs/progress.md](docs/progress.md).
...
- `entity_media_coverage`: one row per entity summarizing image availability, primary image coverage, URL readiness, download state, variant media counts, and media gap reasons.
- `entity_media_gap_queue`: one row per missing image/URL/download reason per entity, prioritized for follow-up image collection and verification.
- `entity_relations`: wiki links with resolved `target_entity_id` values when the target exists in `entities`.
- `entity_link_profiles`: one-row wiki-link graph summaries with resolved/unresolved counts, unique target counts, target-kind distributions, and compact top target lists for entry navigation.
- `entity_prefab_profiles`: one-row prefab/spawn-code summaries with source fields, code categories, and flags for upgraded, reskin, mast-upgrade, chest-upgrade, and merm-upgrade prefab codes.
...
- `entity_completeness_audit`: one row per entity combining source, media, facts, stats, variants, relationships, and official evidence into a readiness score, missing requirements, and next-action list.
- `entity_completeness_gap_queue`: one row per missing completeness requirement per entity, prioritized for source alignment, media collection, stat parsing, variants, relationship resolution, and official verification.
...
- `entity_profile_json`: one gzip-compressed JSON profile per entity for direct wiki/API consumption, aggregating identity, coverage, completeness audit, source coverage, raw/normalized infobox attributes, media, media profile, media coverage, stats, stat rollups, combat profile, food profile, item profile, world profile, character profile, creature profile, recipe profile, variants, categories, facts, recipes, typed relationships, wiki-link profile, prefab profile, taxonomy, and official mentions. Use `dst_wiki_db.entity_profiles.load_profile_json` to decode `profile_json`.
...
- `official_record_mentions`: high-confidence links from official Steam/Klei records back to matching wiki entities, using conservative title and alias/prefab phrase matching.
- `entity_official_verification_queue`: one row per entity still missing official mentions, with priority, Klei/Steam search queries, and the best available wiki counterpart page.
...
- `entity_source_coverage`: one row per entity summarizing wiki.gg/Fandom coverage status, matched page counts, missing source keys, and the best matched source page.
- `entity_source_gap_queue`: one row per missing core source per entity, with priority, available source keys, and the best available source page for follow-up verification.
...
- Cross-source title-index matching currently covers wiki.gg and Fandom with zero source-page gaps. The remaining work is deeper page-diff verification and more granular relationship families beyond title/page correspondence.
