...
- `entity_profile_json`: one gzip-compressed JSON profile per entity for direct wiki/API consumption, aggregating identity, coverage, raw/normalized infobox attributes, media, media profile, stats, stat rollups, combat profile, food profile, item profile, world profile, character profile, creature profile, recipe profile, variants, categories, facts, recipes, typed relationships, wiki-link profile, prefab profile, taxonomy, and official mentions. Use `dst_wiki_db.entity_profiles.load_profile_json` to decode `profile_json`.
